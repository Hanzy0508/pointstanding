...
function buildStandings(captains, allResults, ftName, matchCount) {
  ...
  rows.sort((a, b) => {
    if (b.totalPts !== a.totalPts) return b.totalPts - a.totalPts; // total poin tertinggi duluan
    if (b.booyah !== a.booyah) return b.booyah - a.booyah;         // tiebreak: booyah terbanyak
    return b.totalKills - a.totalKills;                              // tiebreak: kill terbanyak
  });
...
function buildBreakdown(rows, matchCount) {
  ...
  rows.forEach(row => {
    ...
    for (let m = 0; m < matchCount; m++) {
      ...
      if (md && md.found) {
        html += `<td>#${md.rank}</td><td>${md.kills}</td><td style="color:#7aabff;">${md.rankPts + md.kills}</td>`;
      } else {
...
async function extractMatch(matchIdx, slotA, slotB, captains) {
  ...
  const b64A = await toBase64(slotA.file);
  const mimeA = slotA.file.type || 'image/jpeg';
  imageContent.push({ type: 'image', source: { type: 'base64', media_type: mimeA, data: b64A } });

  // slot B — optional
  if (slotB) {
    const b64B = await toBase64(slotB.file);
    const mimeB = slotB.file.type || 'image/jpeg';
    imageContent.push({ type: 'image', source: { type: 'base64', media_type: mimeB, data: b64B } });
  }

  const prompt = `Ini adalah screenshot hasil akhir Free Fire Match ${matchIdx + 1}.
${slotB ? 'Ada 2 gambar: gambar pertama adalah sisi kiri layar, gambar kedua adalah sisi kanan layar.' : 'Ada 1 gambar hasil match.'}

Daftar nama kapten yang harus dicari: ${capList}

TUGAS:
1. Baca semua nama pemain dari gambar (termasuk kedua sisi jika ada 2 gambar)
2. Setiap kapten punya tim. Cari nama kapten atau nama yang paling mirip di antara semua pemain.
3. Catat POSISI/RANK tim kapten tersebut (angka 1-12).
4. Catat TOTAL KILL semua anggota tim kapten di match ini.
5. Jika nama kapten tidak ditemukan sama sekali → rank: 0, kills: 0, found: false

CATATAN:
- Nama mungkin sedikit berbeda (spasi, karakter khusus, huruf besar/kecil) — cari yang paling mirip.
- Setiap tim biasanya terdiri dari 2-4 pemain, jumlahkan semua kill anggota timnya.
- Kembalikan HANYA JSON mentah, tanpa markdown, tanpa backtick, tanpa penjelasan apapun.
...
{"match":${matchIdx + 1},"results":[{"captain":"nama","rank":angka,"kills":angka,"found":true_atau_false}]}`;
  ...
  try {
    ...
    throw new Error(`Gagal parse JSON match ${matchIdx + 1}: ${clean.slice(0, 100)}`);
  }
}
